...
locals {
  ...
  bronze_checkpoint_prefix    = trim(var.bronze_checkpoint_prefix, "/")
  ...
  bronze_checkpoint_path        = "s3://${aws_s3_bucket.lake.bucket}/${local.bronze_checkpoint_prefix}/valid"
  bronze_rejected_checkpoint_path = (
    "s3://${aws_s3_bucket.lake.bucket}/${local.bronze_checkpoint_prefix}/rejected"
  )
  ...
  bronze_glue_job_name       = "${local.name_prefix}-bronze-market-candles-streaming"
